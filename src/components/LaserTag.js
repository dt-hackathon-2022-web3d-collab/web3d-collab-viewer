export class LaserTag {
  ...
  point(from, to) {
    ...
    beam.position.copy(new THREE.Vector3().addVectors(from, to).multiplyScalar(0.5));
    beam.scale.setY(from.distanceTo(to));
    const dir = new THREE.Vector3().subVectors(to, from).normalize();
    beam.setRotationFromQuaternion(new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), dir));
  }
